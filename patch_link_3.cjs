...
const newLinkCode = `const CustomBtnLink = () => {
  ...
  const applyLink = (e: React.MouseEvent | React.KeyboardEvent) => {
    ...
    if (savedRange) {
      ...
      const contentEditable = node && node.closest ? node.closest('[contenteditable]') : null;
      ...
      if (url) {
        // Fallback for browsers that don't fully support createLink natively without strict selection
        let linkUrl = url;
    ...
    <div className="relative inline-flex items-center">
      ...
      {showInput && (
        <div 
          ...
          onMouseDown={(e) => e.stopPropagation()} // Prevent editor from losing focus unexpectedly
        >
          <input
            type="url"
            value={url}
...
// Also make sure z-index of .rsw-toolbar is high enough and .rsw-editor is really visible
// I'll update styles in case they are overridden
code = code.replace('className="w-full bg-white rounded-md border border-gray-200 focus-within:ring-2 focus-within:ring-blue-500/20 focus-within:border-blue-500 relative"', 'className="w-full bg-white rounded-md border border-gray-200 focus-within:ring-2 focus-within:ring-blue-500/20 focus-within:border-blue-500 relative z-10"');
