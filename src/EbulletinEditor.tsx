...
export function EbulletinEditor({ onBack }: { onBack: () => void }) {
  ...
  const [data, setData] = useState({
    ...
    sectionOrder: ['news', 'event', 'report', 'survey', 'research', 'aihub', 'awards', 'podcast', 'webcasts', 'virtual', 'bottomAd'],
  ...
  const allSectionDefs = {
    'header': { id: 'header', title: 'Header & Branding', icon: <CalendarIcon className="w-5 h-5 text-blue-500" />, color: 'bg-blue-500' },
    'news': { id: 'news', title: 'HR in the News & Ad', icon: <Megaphone className="w-5 h-5 text-gray-400" />, color: 'bg-red-500' },
    'event': { id: 'event', title: 'Featured Event', icon: <Clock className="w-5 h-5 text-gray-400" />, color: 'bg-red-500' },
    'report': { id: 'report', title: 'Featured Report', icon: <FileText className="w-5 h-5 text-gray-400" />, color: 'bg-indigo-900' },
    'survey': { id: 'survey', title: 'Spotlight Survey', icon: <BarChart className="w-5 h-5 text-gray-400" />, color: 'bg-indigo-900' },
    'research': { id: 'research', title: 'HR Research Institute', icon: <FileText className="w-5 h-5 text-gray-400" />, color: 'bg-indigo-900' },
    'aihub': { id: 'aihub', title: 'HR & AI Hub', icon: <Monitor className="w-5 h-5 text-gray-400" />, color: 'bg-indigo-500' },
    'awards': { id: 'awards', title: 'Awards Section', icon: <Globe className="w-5 h-5 text-gray-400" />, color: 'bg-orange-500' },
    'podcast': { id: 'podcast', title: 'Podcast Spotlight', icon: <Smartphone className="w-5 h-5 text-gray-400" />, color: 'bg-teal-600' },
    'webcasts': { id: 'webcasts', title: 'Featured Webcasts', icon: <Monitor className="w-5 h-5 text-gray-400" />, color: 'bg-red-500' },
    'virtual': { id: 'virtual', title: 'Upcoming Virtual Events', icon: <CalendarIcon className="w-5 h-5 text-gray-400" />, color: 'bg-indigo-500' },
    'bottomAd': { id: 'bottomAd', title: 'Bottom Ad', icon: <ImageIcon className="w-5 h-5 text-gray-400" />, color: 'bg-gray-500' }
  };

  const sectionDefs = [
    allSectionDefs['header'],
    ...(data.sectionOrder || ['news', 'event', 'report', 'survey', 'research', 'aihub', 'awards', 'podcast', 'webcasts', 'virtual', 'bottomAd']).map(id => allSectionDefs[id as keyof typeof allSectionDefs])
  ].filter(Boolean);
  ...
  const moveSection = (id: string, direction: 'up' | 'down', e: React.MouseEvent) => {
    ...
    setData(prev => {
      const newOrder = [...(prev.sectionOrder || ['news', 'event', 'report', 'survey', 'research', 'aihub', 'awards', 'podcast', 'webcasts', 'virtual', 'bottomAd'])];
                ...
                >
                  ...
                  <div className="flex items-center gap-2 text-gray-400">
                    {!isExpanded && (
                      <>
                        <button 
                          onClick={(e) => moveSection(sectionDef.id, 'up', e)}
                          disabled={sectionDef.id === 'header' || (data.sectionOrder && data.sectionOrder.indexOf(sectionDef.id) === 0)}
                          className={`p-1 rounded ${sectionDef.id === 'header' || (data.sectionOrder && data.sectionOrder.indexOf(sectionDef.id) === 0) ? 'text-gray-200 cursor-not-allowed' : 'text-gray-400 hover:bg-gray-100 hover:text-gray-600'}`}
                        >
                          <ArrowUp className="w-3.5 h-3.5" />
                        </button>
                        <button 
                          onClick={(e) => moveSection(sectionDef.id, 'down', e)}
                          disabled={sectionDef.id === 'header' || (data.sectionOrder && data.sectionOrder.indexOf(sectionDef.id) === (data.sectionOrder ? data.sectionOrder.length - 1 : 10))}
                          className={`p-1 rounded ${sectionDef.id === 'header' || (data.sectionOrder && data.sectionOrder.indexOf(sectionDef.id) === (data.sectionOrder ? data.sectionOrder.length - 1 : 10)) ? 'text-gray-200 cursor-not-allowed' : 'text-gray-400 hover:bg-gray-100 hover:text-gray-600'}`}
                        >
