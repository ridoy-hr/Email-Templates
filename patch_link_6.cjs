...
const newLinkCode = `const CustomBtnLink = () => {
  ...
  const applyLink = (e: React.MouseEvent | React.KeyboardEvent) => {
    ...
    if (savedRange && url) {
      ...
      // If we use execCommand, it preserves formatting and undo history better
      // But if it fails, we fallback to DOM manipulation
      let success = false;
      try {
        if (text.trim() === '') {
      ...
      // Attempt to dispatch input event so react-simple-wysiwyg detects the change
